  ...
  provideCompletionItems(_document, position) {
    ...
    if (this.saveLine !== currentLine) {
      ...
      for (let key in saveDeclarationData) {
        ...
        // 名前が重複するものは追加しない
        const hasSameDataRegistered = addCommplationItems.some(
          (d) => d.label === data.name
        );
      ...
      this.completionItems = defaultCommplationItems.concat(
        addCommplationItems
      );
    }
...
function createMarkdown(data, desc, showHr) {
  ...
  message.appendCodeblock(
    `${scope}${data.type} ${data.name} '(Line ${data.line})`
  );
  if (desc !== null)
    message.appendMarkdown((showHr ? "\n\n***" : "") + `\n\n${desc}`);
  return message;
}
...
function activate(context) {
  ...
  // 起動時にスキャン
  [saveDeclarationData, saveDefineData] = scanSourceCode(
  ...
  const changeActiveEditor = vscode.window.onDidChangeActiveTextEditor(
    (event) => {
      if (event && event._documentData._languageId !== "sb4") return;
